...
# Generated on: 2025-09-25T10:35:13.594
...
struct StyleBase
    ...
    function StyleBase(name::String, category_number::Int64, style_letter::String, style_guide::String, category::String, type::StyleCategories)
        @assert length(style_letter) >= 1 "`style_letter` must have a minimum length of 1"
        @assert length(style_letter) <= 1 "`style_letter` must have a maximum length of 1"
        @assert occursin(Regex(pattern_str), style_letter) "`style_letter` must match the pattern))"
        new(name, category_number, style_letter, style_guide, category, type)
    end
end
